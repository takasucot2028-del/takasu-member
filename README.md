...
2. `gas/Code.gs` の内容をエディタに貼り付け（**コードの書き換えは不要**）
3. **「プロジェクトの設定 → スクリプト プロパティ」** で以下を設定（IDやパスワードをリポジトリに残さないため）:
   - `SPREADSHEET_ID` = スプレッドシートのID（URLの `/d/` と `/edit` の間）
   - `ADMIN_EMAIL` = 事務局ログイン用メール
   - `ADMIN_PASSWORD` = 事務局ログイン用パスワード（**`admin123` 以外の強固なものに**）
   - ※ADMIN_系が未設定だとデモ用 `admin@takasu-sc.jp` / `admin123` で作成されます
4. `setupSpreadsheet()` を1回実行（シート・マスタ・管理者の初期化）
   - 既に実行済みでパスワードを変えたい場合は、プロパティ更新後に `resetAdminCredentials()` を実行
6. 「デプロイ」→「新しいデプロイ」→「ウェブアプリ」として公開
   - 実行ユーザー: 自分 / アクセスできるユーザー: **全員**（フロントが匿名で呼ぶため）
7. デプロイURLをコピー

> **セキュリティ**: Web App は匿名アクセスで公開しますが、`gas/Code.gs` がアプリ独自の
> トークン（ログイン時に発行、CacheService で6時間保持）で認可します。会員一覧・請求などの
> 事務局用エンドポイントは管理者トークンがないと実行できず、会員本人は自分の情報のみ取得・更新可能です。
